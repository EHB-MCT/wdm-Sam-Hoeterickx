...
export const GeoLocationVisual = ({ geoLocationData = [] }) => {
    ...
    const heatmapData = useMemo(() => {
        ...
        geoLocationData.forEach((location) => {
            ...
            const gridLng =
                Math.floor(location.longitude / gridSize) * gridSize;
    ...
    const getMapBounds = () => {
        ...
        const bounds = [
            [
                Math.min(...heatmapData.map((p) => p.latitude)),
                Math.min(...heatmapData.map((p) => p.longitude)),
            ],
            [
                Math.max(...heatmapData.map((p) => p.latitude)),
                Math.max(...heatmapData.map((p) => p.longitude)),
            ],
        ];
    ...
    const stats = useMemo(() => {
        const uniqueSessions = new Set(geoLocationData.map((d) => d.sessionId))
            .size;
        ...
        return (
            <div className="geo-data-container geo-data-empty">
                <div className="geo-data-empty-content">
                    ...
                    <h3 className="geo-data-empty-title">
                        Geen locatie data beschikbaar
                    </h3>
                    <p className="geo-data-empty-text">
                        Wachten op coördinaten...
                    </p>
                </div>
            </div>
        );
                            ...
                            >
                                <Popup>
                                    <div className="geo-popup-content">
                                        <strong className="geo-popup-title">
                                            Locatie Cluster
                                        </strong>
                                        <span className="geo-popup-info">
                                            Aantal punten:{" "}
