...
export const BrowserDataVisual = ({ browserData = [] }) => {
    ...
    // 3. Extensions Data (Nieuw)
    const extensionsData = useMemo(() => {
        const counts = {};
        
        browserData.forEach(session => {
            // Check of extensions bestaat en een array is
            if (Array.isArray(session.extensions)) {
                session.extensions.forEach(ext => {
                    // Gebruik extensie naam of ID
                    const extName = typeof ext === 'string' ? ext : ext?.name || 'Unknown Extension';
        ...
        return Object.entries(counts)
            ...
            .sort((a, b) => b.count - a.count); // Sorteer op populariteit
    }, [browserData]);
    ...
    if (!Array.isArray(browserData) || browserData.length === 0) {
        ...
        <div className="browser-data-container browser-data-empty">
            ...
            <h3 className="browser-data-empty-title">Geen browser data beschikbaar</h3>
            <p className="browser-data-empty-text">Er zijn nog geen sessies opgenomen.</p>
            </div>
        </div>
        );
    }
    ...
    return (
        <div className="browser-data-container">
            ...
            <div className="browser-data-header">
                ...
                <p className="browser-data-subtitle">Overzicht van browser, platform en extensie statistieken</p>
            </div>
            ...
            <div className="browser-data-stats">
                ...
                <div className="stat-card stat-card--pink">
                    ...
                    <div className="stat-content">
                        <p className="stat-value">{browserDistribution.length}</p>
                        <p className="stat-label">Browsers</p>
                    </div>
                </div>

                {/* Nieuwe Stat Card voor Extensions */}
            ...
            <div className="browser-data-tabs">
                ...
                <button
                    ...
                    className={`dashboard-tab ${activeView === 'platform' ? 'dashboard-tab--active' : ''}`}
                >
                    Platform Overzicht
                </button>
                <button
                    onClick={() => setActiveView('extensions')}
                    className={`dashboard-tab ${activeView === 'extensions' ? 'dashboard-tab--active' : ''}`}
                >
                    Extensies
                </button>
            </div>
            ...
            {activeView === 'overview' && (
                <div className="browser-data-visualisations">
                    <div className="browser-data-chart">
                        <h3 className="browser-data-chart-title">Browser Gebruik</h3>
                        ...
                        <BarChart data={browserDistribution}>
                            ...
                            <Tooltip />
                            <Legend />
                            <Bar dataKey="sessies" fill="#3b82f6" radius={[8, 8, 0, 0]} />
                        </BarChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    <div className="browser-stats-grid">
                        ...
                        <div key={index} className="browser-stat-card">
                            ...
                            <h4 className="browser-stat-name">{browser.name}</h4>
                            <Globe className="browser-stat-icon" style={{ color: COLORS[index % COLORS.length] }} />
                            </div>
                            <p className="browser-stat-value" style={{ color: COLORS[index % COLORS.length] }}>
                            {browser.sessies}
                            </p>
                            ...
                            <div 
                                ...
                                style={{ 
                                width: `${(browser.sessies / browserData.length) * 100}%`,
                                backgroundColor: COLORS[index % COLORS.length]
                                }}
                            />
                            </div>
                            <p className="browser-stat-percentage">
                            {((browser.sessies / browserData.length) * 100).toFixed(1)}% van totaal
                            </p>
                        </div>
                        ))}
                    </div>
            ...
            {activeView === 'platform' && (
                <div className="browser-data-platform">
                    <h3 className="browser-data-platform-title">Platform Statistieken</h3>
                    <div className="browser-data-platform-grid">
                        ...
                        {platformData.map((platform, index) => (
                            ...
                            <div className="platform-stat-header">
                                ...
                                <span className="platform-stat-value">{platform.value}</span>
                            </div>
                            <div className="platform-stat-progress">
                                <div 
                                className="platform-stat-progress-bar"
                                style={{ width: `${(platform.value / browserData.length) * 100}%` }}
                                />
                            </div>
                            <p className="platform-stat-percentage">
                                {((platform.value / browserData.length) * 100).toFixed(1)}% van alle sessies
                            </p>
                            </div>
                        ))}
                        </div>
            ...
            {/* VIEW 3: EXTENSIONS (NIEUW) */}
            {activeView === 'extensions' && (
                <div className="browser-data-visualisations">
                    <div className="browser-data-chart">
                        <h3 className="browser-data-chart-title">Meest Gedetecteerde Extensies</h3>
                        {extensionsData.length > 0 ? (
                            <ResponsiveContainer width="100%" height={400}>
                                <BarChart layout="vertical" data={extensionsData.slice(0, 10)}> {/* Top 10 */}
                                    <CartesianGrid strokeDasharray="3 3" />
                                    <XAxis type="number" allowDecimals={false} />
                                    <YAxis type="category" dataKey="name" width={150} />
                                    <Tooltip />
                                    <Legend />
                                    <Bar dataKey="count" name="Aantal gebruikers" fill="#f59e0b" radius={[0, 4, 4, 0]}>
                            ...
                            <div className="p-8 text-center text-gray-500 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                ...
                                <p>Geen extensie data gevonden in de huidige sessies.</p>
                            </div>
                        )}
                    </div>

                    <div className="browser-data-platform-list">
                        <h4 className="font-semibold text-gray-700 mb-4">Extensie Detailijst</h4>
                        {extensionsData.length > 0 ? (
                            extensionsData.map((ext, index) => (
                                <div key={index} className="platform-stat-item">
                                    <div className="platform-stat-header">
                                        ...
                                        <span className="platform-stat-value">{ext.count}</span>
                                    </div>
                                    <div className="platform-stat-progress">
                                        <div 
                                            className="platform-stat-progress-bar"
                                            style={{ 
                                                width: `${(ext.count / browserData.length) * 100}%`,
                                                backgroundColor: COLORS[index % COLORS.length]
                                            }}
                                        />
                                    </div>
                                    <p className="platform-stat-percentage">
                                        {((ext.count / browserData.length) * 100).toFixed(1)}% van gebruikers
                                    </p>
                                </div>
                            ))
                        ) : (
                            <p className="text-sm text-gray-500 italic">Geen extensies gedetecteerd.</p>
                        )}
                    </div>
                </div>
            )}
            ...
            <div className="browser-data-table">
                ...
                <table className="browser-data-table-content">
                    ...
                    <tr className="browser-data-table-header">
                        <th className="browser-data-table-th">Session ID</th>
                        <th className="browser-data-table-th">Platform</th>
                        <th className="browser-data-table-th">Browser</th>
                        <th className="browser-data-table-th">Extensies</th>
                    </tr>
                    </thead>
                    <tbody>
                    {browserData.map((session) => (
                        <tr key={session._id} className="browser-data-table-row">
                        <td className="browser-data-table-td browser-data-table-id">
                            {session.session_id.substring(0, 10)}...
                        </td>
                        <td className="browser-data-table-td">{session.platform}</td>
                        <td className="browser-data-table-td">
                            {session.brand?.brand || 'Unknown'} <span className="text-xs text-gray-400">({session.brand?.version})</span>
                        </td>
                        <td className="browser-data-table-td">
                            {Array.isArray(session.extensions) && session.extensions.length > 0 ? (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800">
                                    {session.extensions.length} geïnstalleerd
                                </span>
                            ) : (
